...
import { deleteResearch, editResearch, getResearches } from "../../store/slices/researchesSlice";
...
const MyResearches = () => {
  ...
  const isResearDeleted = useSelector((state) => state.researchesData.isResearDeleted ) ;
  const isResearchEditSuccess = useSelector((state) => state.researchesData.isResearchEditSuccess ) ;
  ...
  useEffect (() => {
    ...
    if (isResearDeleted.status) {
      Swal.fire({
        icon: 'success',
        title: isResearDeleted.message,
  ...
  useEffect (() => {
    if (isResearchEditSuccess.status) {
      Swal.fire({
        icon: 'success',
        title: isResearchEditSuccess.message,
        showConfirmButton: false,
        timer: 3000
      })
      setTimeout(() => {
  ...
  const handleSubmit = (e) => {
    handleClose()
    e.preventDefault();
    dispatch(editResearch({name : title , teacher_name: supervisor, subjecet_id : selectedSubject,notes:notes, id})) ;
                    ...
                    <TableData style={{display:'flex'}}>
                      {
                           obj.status==1 ? 
                           <>
                           <GrantButton onClick={()=> dispatch(deleteResearch(obj.id))  }  >حذف</GrantButton>  
